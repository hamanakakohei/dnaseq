...
def select_one_family(df, proband, others, controls):
  ...
  for _, row in df.iterrows():
    ...
    more_expanded_all.append(  sum(c > proband_count for    c in count_dict.values()) )
    more_expanded_ctrl.append( sum(c > proband_count for s, c in count_dict.items() if s in controls))

  # 作ったリストを基の表に列として追加する
  # サンプル名を列名にして、motif/locus read countの値を入れる
  df['NofLongerExpInAllSamples'] = more_expanded_all
  df['NofLongerExpInCtlSamples'] = more_expanded_ctrl
...
def rename_motif_proband( motif_proband, members ):
  ...
  motif_proband = motif_proband.\
    rename({
      ...
      'NofLongerExpInAllSamples': 'NofLongerExpInAllSamples_motif',
      'NofLongerExpInCtlSamples': 'NofLongerExpInCtlSamples_motif'
    }, axis=1)
...
def run():
  ...
  pd.merge( locus_proband, motif_proband, how='outer' ).\
    drop(['high_case_counts','counts','high_case_counts__motif','counts__motif'], axis=1).\
    assign(
      start                          = lambda d: d["start"                         ].astype("Int64"),
      end                            = lambda d: d["end"                           ].astype("Int64"),
      NofLongerExpInAllSamples       = lambda d: d["NofLongerExpInAllSamples"      ].astype("Int64"),
      NofLongerExpInCtlSamples       = lambda d: d["NofLongerExpInCtlSamples"      ].astype("Int64"),
      NofLongerExpInAllSamples_motif = lambda d: d["NofLongerExpInAllSamples_motif"].astype("Int64"),
      NofLongerExpInCtlSamples_motif = lambda d: d["NofLongerExpInCtlSamples_motif"].astype("Int64"),
    ).\
    to_csv( args.out, sep='\t', index=False, na_rep="NA" )
